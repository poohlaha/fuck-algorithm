...
12. 正则表达式匹配
    ...
    1 判断是否可以用 DP
    1.1 最优子结构
        求解 s[0..i) 和 p[0..j) 的匹配结果, 取决于:
        - 当 p[j - 1] != '*' 时, 判断 s[0..i-1] 和 p[0..j-1] 是否匹配
        - 当 p[j - 1] == '*' 时, 表示: 前一个字符 p[j-2] 可以出现 0 次、1 次、或任意多次
          - 前一个字符出现 0 次:
            - p[j-2]：与 * 配套的前一个字符
            - p[j-1] = '*'：当前考虑的 *
            - 忽略 p[j-2] 和 p[j-1]（x* 视作不出现）(要让 “p[j-2] * 出现 0 次”，相当于从模式中去掉这两个字符，继续匹配)
          - 前一个字符出现至少 1 次:
            - 当前字符 s[i-1] 与 p[j-2] 相同，或者 p[j-2] == '.'
            - 忽略 s[i-1]
    1.2 重叠子问题
        匹配 s[0..i] 和 p[0..j]，可能在不同路径中多次计算
    1.3 无后效性
        判断 s[0..i] 和 p[0..j] 是否匹配，只与当前的字符和之前子问题的匹配状态有关，不依赖于匹配路径的选择顺序
    
    2. 定义状态
       什么是状态?
       - 状态是子问题的标识，用来唯一描述子问题，让我们知道当前计算的“场景”是什么。
    
       要求 s[0..i) 和 p[0..j) 的匹配结果, 化解为问题 s[0..i-1] 和 p[0..j-1] 是否匹配
       因此定义: dp[i][j] =  s[0..i-1] 和 p[0..j-1] 是否匹配
       dp[i][j] 表示的是: 字符串 s 的前 i 个字符（s[0..i-1]）是否能被模式 p 的前 j 个字符（p[0..j-1]）完全匹配
    
    3. 列出选择 (Choice)
       求 s[0..i) 和 p[0..j) 是否匹配, 取决于:
       - 当 p[j - 1] != '*' 时
         - 如果 s[i - 1] = p[j - 1] 或 p[j - 1] = '.', 则问题简化为: dp[i][j] = dp[i - 1][j - 1]
         - 否则: dp[i][j] = false
       - 当 p[j - 1] == '*' 时
         - 前一个字符 p[j - 2] 出现 0 次, 跳过最后两字符 p[j-2] 和 p[j-1], 则问题简化为 dp[i][j] = dp[i][j - 2]
         - 前一个字符 p[j - 2] 出现至少 1 次, 当 s[i - 1] 和 p[j -2] 相同 或 p[j-2] == '.', 则问题简化为: dp[i][j] = dp[i - 1][j]
      
    4. 写出转移方程(递推公式)
       将“选择”转化为最优性方程: 
      
       当 p[j - 1] != '*' 时: 
       - s[i - 1] = p[j - 1] 或 p[j - 1] = '.', dp[i][j] = dp[i - 1][j - 1] 
       - 否则: dp[i][j] = false
    
       当 p[j - 1] == '*' 时: 
       - * 表示前一个字符出现 0 次, dp[i][j] = dp[i][j - 2]
       - * 表示前一个字符出现 至少 1 次, dp[i][j] = dp[i - 1][j]
       ...
       1) 处理 c*
          c* 出现 0 次 → 跳过，模式变为: p = "a*b", 字符串仍是: s = "aab"
       2) 处理 a*
          (1) 让 a* 出现 0 次
              跳过 a*，模式变为, 模式变为: p = "b", 字符串仍是: s = "aab", 此时 s 开头是 'a' 而模式开头是 'b'，无法匹配，路径失败。
          (2) 让 a* 出现至少 1 次
              检查 s[0] == 'a' 与 p[0] == 'a' 匹配，匹配成功。
              消耗掉 s[0]，继续匹配剩余:
              s = "ab"
              p = "a*b"
          因为 a* 可以继续匹配
         ...
         此时再次遇到 a*:
            尝试让 a* 出现 0 次:
               跳过 a*，模式变为: p = "b", 字符串中: s = "b", 两者首字符都是 'b'，匹配成功, 都消耗完后，完成匹配。
      
    5. 边界条件(初始化)
       dp[0][0] = true, 表示空字符串 s 与空模式 p 匹配
       dp[0][j] 
       - 当字符串 s 为空，模式 p 不为空时，只有当 p 能表示 空字符串 才可能匹配, dp[0][j] = dp[0][j - 2] (p[j - 1] = '*')
       - 否则为 dp[0][j] = false
    
    6. 计算顺序
       由于 dp[j][j] 依赖 dp[i - 1][j - 1]、dp[i][j - 2]、dp[i - 1][j]
       需要保证依赖的状态在更新前 已被计算，因此:
       - 外层循环：i 从 0 到 m (表示 s 的长度)
       - 内层循环：j 从 1 到 n (表示 p 的长度，j = 0 已初始化)
    
    7. 返回结果
       要求字符串 s 是否能被模式 p 完全匹配覆盖, 即: dp[m][n]
    ...
    此时的 p 的 a* 部分可以考虑用: (0 次, 1次, 多次)
    
    1. 如果用 0 次:
       p 的 a* 部分可以直接抛弃(想象成消消乐, 并同有消去 s 中的字符, 自己损失两位)
       s = # # a
               i
       p = # # a *
             j    
       ...
       - 不使用 a* 来匹配任何字符
       - 直接将 a* 视为消失(消消乐)
       - 抛弃模式的两位: p[j-2] 和 p[j-1]
    
       状态转移: 
               dp[i][j] = dp[i][j-2] (当前字符未被匹配，模式去掉 a* 后是否匹配)
    
    2. 如果用 1 次:
       p 的 a* 部分消去了 s 的一个字符(p 中的 a* 功能用完了, 也应该丢弃 -- 左移 2位)
       s = # # a
             i
       p = # # a *
             j 
       ...
       含义:
       - 使用 a* 来匹配当前 s[i-1], 且只匹配一次就结束 a* 的功能
       - 因此:
             字符串 i 向前移动一位(消除被匹配字符)
             模式 j 向前移动两位(a* 完整消失)
    
       状态转移:
              dp[i][j] = dp[i-1][j-2](表示 a* 刚好匹配一个字符并消失后的匹配情况)
    
       
       其实: dp[i][j] = dp[i][j-1]，其实是一样的，因为此时 s[i] == p[j-1]，即下一步就是，dp[i][j-1] == dp[i-1][j-2]
       其实把 a* 理解为一个整体，要消去就全部消去，比较直观一点
    
    3. 如果用多次
       p 的 a* 部分消去了 s 的末尾字符后, 还要继续起作用, 继续消除 s 前面的字符(不移动 p 的指针)
       s = # # a
             i
       p = # # a *
                 j
       ...
       状态转移:
              dp[i][j] = dp[i-1][j](表示使用 a* 匹配当前字符后继续使用 a* 匹配剩余字符串)
    
    4. 总结:   
       dp[i][j] = dp[i][j-2](出现 0 次, a* 被视作不存在)
       dp[i][j] |= dp[i-1][j](出现 >=1 次, 匹配 s[i-1] 后继续使用 a* 匹配剩余字符串)(dp[i][j] = dp[i][j] || dp[i - 1][j])
    ```