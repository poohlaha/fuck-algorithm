...
### DP 维数确定
    `DP 的维数 = “必须同时记住”的独立状态变量个数`

1. 先把「维数」和「数组」彻底区分开
   `维数 = 状态变量的数量`, 有时候三维状态可以用结构体或两个数组表示，但 `逻辑上仍然是三维`

2. 判断 DP 维数的通用三步法(黄金法则)
   - 问自己一句话(核心)
     `为了避免重复计算，我在递归/转移时，必须“记住”哪些信息？`
     - 每一个 `“必须记住”且互相独立的变量` - `一维`
   - 对每个变量问一句
     `如果我不记录它，后续决策会不会出错？`
     - 会出错 → 必须成为一维
     - 不会出错 → 不需要
   - 能合并吗？
     如果两个变量 `永远同步变化`, 可以 `合并成一维`, 否则 `❌ 不能`
    
3. 一个超重要的判断口诀
   `遇到 DP，数清楚以下三类东西`, `每一类，几乎必占一维`
   - 我在哪？（位置 / 节点 / 下标）
   - 我处在什么状态？（是否选过、是否激活、是否折扣）
   - 我消耗了什么资源？（时间 / 金钱 / 次数 / 容量）