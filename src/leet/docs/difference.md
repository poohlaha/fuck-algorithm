# 差分数组
  一种高效执行 `区间修改` 的数据结构, 相比 `前缀和(Prefix Sum)` 适合用于 `区间查询`, 差分数则非常适用于 `频繁修改数组某个区间的值`  的场景。
...
1. 公式
  ...
  ```text
   ...
   D[x1][y1]         += c  // 左上(主对角线起点, 启发传播)
   D[x1][y2 + 1]     -= c  // 右上(副对角线, 右边一列清除)
   D[x2 + 1][y1]     -= c  // 左下(副对角线, 下边一行清除)
   D[x2 + 1][y2 + 1] += c  // 右下(主对角线右下角, 修正补回来)
