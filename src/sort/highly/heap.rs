    ...
    fn sort(&mut self) {
        ...
        /**
          ...
          1. 数组表示的完全二叉树为:
               ...
               /  \
              5    1
          2. len = 5，最后一个非叶子节点的索引为 len / 2 - 1 = 1
          3. 从索引 1 开始调整堆
            - 调整索引 1：交换 10 和 1，堆变为 [4, 5, 3, 10, 1]
            - 调整索引 0：交换 4 和 10，堆变为 [10, 5, 3, 4, 1]
          4. 如果从索引 0 开始（从上往下），则调整的顺序可能会破坏子树
        */
        ...
        假设数组初始为 [10, 9, 8, 3, 2, 5]，已经构建为大顶堆
           ...
           3   2 5
         1. 第一次交换
            - 交换堆顶 10 和最后一个元素 5，数组变为 [5, 9, 8, 3, 2, 10]。
            - 此时，10 已经排到正确位置（末尾）。
         2. 调整剩余部分为堆
            - 调整范围 [5, 9, 8, 3, 2]
            - 调整后堆为:
        ...
        */
        for i in (1..size).rev() {
            // 或直接使用上浮, 不需要反转数组
            self.data.swap(0, i); // 交换 `堆顶(最大值)` 与`堆尾` 元素
            size -= 1;
            self.sink(size, 0) // 堆顶元素(索引 0 的元素)可能不再是剩余堆的最大值，需要从堆顶开始重新调整堆
        }
    }
