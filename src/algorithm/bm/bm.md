...
2. 两大启发式规则
   - 坏字符规则(Bad Character Rule)
     ...
     3. 所以可以跳过一定距离, 使用 `pattern[2]`` 对齐 `text[i]`(或更远)
     ```
...
7. 举例
   ...
   -> 匹配好后缀规则
   -> 好后缀规则1: 查找 suffix[k] 是否为 -1, suffix[8 - 7] = suffix[1] = 0, 满足: y = j - suffix[k] + 1 = 7 - 0 + 1 = 8
   -> max(x, y) = max(-1, 8) = 8
   -> 滑动距离: 8, i += 8 = 13
   ...
   -> 匹配好后缀规则
   -> 好后缀规则1: 查找 suffix[k] 是否为 -1, suffix[8 - 8] = suffix[0] = -1, 不满足, 触发好后缀规则2
   -> 好后缀规则2: 查找 prefix[k] 是否为 true, prefix[8 - 8] = false, 不满足
   -> 好后缀规则2没匹配, 则整体滑动模式串长度: y = m, y = 9
   -> 取 max(x, y) = max(3, 9) = 9
   -> 滑动距离: 9, i += 9 = 22
...
### 预处理阶段
1. 构建 `坏字符规则表 H`
   `H[]` 取 `最后一次出现的索引位置`, 即: `H[c] = last_index(c)`, `不存在` 取默认值 `-1`

2. 构建 `好后缀规则表`
  ```markdown
   2.1 定义两辅助函数 `suffix 数组` 和 `prefix 数组`, 用于确定 `当坏字符规则失效时，模式串应该向右移动多远`
       2.1.1 定义 `suffix 数组`(长度为 i 的好后缀，在模式串中 `另一个匹配子串` 的起始下标), 默认值为 `-1`, 长度为 `m`
       2.1.2 定义 `prefix 数组`(模式串的长度为 i 的后缀，是否同时也是模式串的前缀), 默认值为 `false`, 长度为 `m`
       
   2.2 定义 `i = m - 2`(需要去除末尾), 从 `右向左` 循环
       2.2.1 定义临时变量 `j = i`, `k = 0`     
       2.2.2 比较 `pattern[j]` 和 `pattern[m - 1 - k]` 是否相等
         - 相等, `i -= 1`, `k += 1`, 继续比较 `pattern[j]` 和 `pattern[m - 1 - k]` 是否相等
         - 不相等, 没有任何后缀匹配成功, `suffix` 和 `prefix` `不变`
       2.2.3 匹配到结果(k > 1), 设置 `suffix[k] = i + 1`, 表示 `长度为 k 的后缀 x，在 pattern 的第 i 个位置出现过`  
       2.2.4 匹配到了开头, 设置 `prefix[k] = true`, 表示 `模式串的长度为 k 的“后缀” == 模式串的“前缀”`, 即: `模式串的后 k 个字符` 和 `和 模式串的前 k 个字符` `完全相同`, 如: `ABA` 和 `A`
  ```
...
2. 开始循环
   ...
   2.2 取出 `text[i .. m - 1 + i]` 和 `pattern` 比较
       2.2.1 定义临时变量 `j = m - 1`(`m` 为 `pattern` 的长度)
       2.2.2 从 `右向左` `逐个比较` 每个字符, 比较窗口大小为 `[0..j]`
         - 匹配成功(j < 0), `i += 1`, 结束当前循环
         - 匹配失败, 触发 `坏字符` 和 `好后缀规则`
    
       2.2.3 匹配失败
         匹配 `坏字符规则`, 坏字符 `c = text[j]`, 查找是否在 `坏字符规则表 H` 中存在 `c`
           - 定义坏字符规则移动距离 x = 1
           - `坏字符规则表 H` 中存在 `c` 的值 `ch`, 且 `j > ch`, 则 x = j - ch
   
      2.2.4 匹配好后续规则
        好后缀长度 `k` 为 `m - 1 - j`
        - 定义好字符规则移动距离 `y`
        - 好后缀长度 `k` > 0
          - 好后缀规则1: 查找 `suffix[k]` 是否为 `-1`, 如果为 `-1`, 则取 `y = j - suffix[k] + 1`, 如果不为 `-1`, 则走 `好后缀规则2`
          - 好后缀规则2: 查找 `prefix[k]` 是否为 `true`
            - 定义临时变量 `r = k`
            - 递减 `y`
              - 如果 `prefix[k] = true`
                - 将 `模式串前 r 个字符对齐好后缀末尾`, 取 `y = m - r`, 结束循环
              - y -= 1
           - 如果没有任何前缀匹配(`y = 0`), 整体滑动模式串长度 `y = m`
   
       2.2.5 滑动距离
             - 滑动距离: 取 `坏字符规则移动距离 x` 和 `好字符规则移动距离 y` 的最大值: `max(x, y)`
             - `i += 滑动距离`
   ```